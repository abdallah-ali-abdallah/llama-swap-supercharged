...
function appendLog(
	...
	store.update((prev) => {
		...
		return updatedLog.length > LOG_LENGTH_LIMIT
			? updatedLog.slice(-LOG_LENGTH_LIMIT)
			: updatedLog;
	});
}
		...
		apiEventSource.onmessage = (e: MessageEvent) => {
			try {
				...
				switch (message.type) {
					...
					case "activityLive": {
						const liveRows = JSON.parse(message.data) as
							| LiveActivityRow[]
							| null;
...
export async function getPersistenceSettings(): Promise<PersistenceSettings | null> {
	try {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to fetch persistence settings: ${response.status}`,
			);
		}
...
export async function updatePersistenceSettings(
	...
	if (!response.ok) {
		throw new Error(
			`Failed to update persistence settings: ${response.status}`,
		);
	}
	return await response.json();
}
...
export async function getModelConfiguration(
	...
	try {
		const response = await fetch(
			`/api/models/config/${encodeURIComponent(model)}`,
		);
		...
		if (!response.ok) {
			throw new Error(
				`Failed to fetch model configuration: ${response.status}`,
			);
		}
		return await response.json();
	} catch (error) {
...
export async function cancelActivity(id: string): Promise<boolean> {
	try {
		const response = await fetch(`/api/activity/${id}/cancel`, {
			method: "POST",
		});
		if (!response.ok) {
			if (response.status === 404) {
